                ...
                uint2 imageSize, float track_threshold)
{
	...
	TooN::Matrix<8, 32, const float, TooN::Reference::RowMajor> values(output);
        ...
        {
            cerr<<"sqrt "<<values(0, 0)<<" "<<values(0, 28);
            pose = oldPose;
            return false;
        }
        else if(values(0, 28) / (imageSize.x * imageSize.y) < track_threshold)
        {
            cerr<<"Values "<<values(0,28)<<" "<<(imageSize.x * imageSize.y)<<" "<<track_threshold;
		pose = oldPose;
		return false;
	} else {
		return true;
